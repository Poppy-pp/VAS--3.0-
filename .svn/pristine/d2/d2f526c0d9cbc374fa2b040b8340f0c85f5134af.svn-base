    ...
    <div>
                ...
                <el-col :span="18">
                    <el-card>
                        <el-table :data="storagePurchaseList" border highlight-current-row @row-click="tableRowClick"
                            ...
                            <el-table-column prop="modelspec" align="center" label="规格" min-width="70">
                            </el-table-column>
        ...
        <el-dialog :modal-append-to-body="false" :visible.sync="addFormVisible" v-if="addFormVisible" :close-on-click-modal="false">
            <div style="margin: 10px 0; overflow: hidden">
                <span v-for="item in filterPurchaseList" class="purchase-item">{{item.prodspec}}({{item.requestqty}})</span>
            ...
            <el-form :model="addForm" label-width="100px" :rules="addFormRules" ref="addForm">
                <el-collapse v-model="activeCollapse">
                    <el-collapse-item style="margin-bottom: 10px;" :name="index"
                                      v-for="(item, index) in addForm.dataList"
                                      :key="item.contractno">
                        <template slot="title">
                            {{item.contractno}} <el-button style="margin-left: 10px; color: #e04659" type="text" size="mini" @click.stop="removePurchase(index)" v-if="addForm.dataList.length > 1">删除</el-button>
                        </template>
                        <el-card>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click.native="addSubmit" :loading="addLoading" :disabled="addLoading || isDisabled">提交</el-button>
            </div>
        </el-dialog>
    </div>
</template>
