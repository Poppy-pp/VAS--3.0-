<template>
	<section class="tab_content-wrapper">
		...
		<el-table :max-height="windowOutHeight-275" border :data="listData" :row-class-name="tableRowClassName" highlight-current-row v-loading="listLoading" >
			...
			<el-table-column prop="iccid" label="ICCID" width="180" align="center" >
			</el-table-column>
			<el-table-column prop="modelname" label="型号" align="center" width="170" >
			</el-table-column>
			<el-table-column prop="issueplace" label="发放地区" align="center" >
			</el-table-column>
			<el-table-column prop="operatorname" label="运营商" align="center" >
			</el-table-column>
			...
			<el-table-column prop="createdate" label="入库日期" align="center" width="110" :formatter="dateFormatter">
			</el-table-column>
		...
		<el-col :span="24" class="toolbar">
			<el-button type="danger" @click="handleAddBatch" icon="el-icon-plus">批量入库</el-button>
		...
		<el-dialog title="SIM卡详情" :modal-append-to-body="false" :visible.sync="formDialogTableVisible" class="details">
			...
			<el-row v-if="SIMdata !== null">
				...
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
						<dt>创建人:</dt>
						<dd>{{SIMdata.createbyname == undefined ? '--' : SIMdata.createbyname}}</dd>
					</dl>
				</el-col>
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
				...
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
						<dt>修改人:</dt>
						<dd>{{SIMdata.updatebyname == undefined ? '--' : SIMdata.updatebyname}}</dd>
					</dl>
				</el-col>
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
			...
			<el-row v-if="Proddata !== null ">
				...
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
						<dt>创建人:</dt>
						<dd>{{Proddata.createbyname == undefined ? '--' : Proddata.createbyname}}</dd>
					</dl>
				</el-col>
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
				...
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
						<dt>修改人:</dt>
						<dd>{{Proddata.updatebyname == undefined ? '--' : Proddata.updatebyname}}</dd>
					</dl>
				</el-col>
				<el-col :span="6">
					<dl class="dllist" style="margin-bottom:10px;">
			...
			<el-row v-else-if="Proddata == null && SIMdata == null">暂无详情</el-row>
		</el-dialog>
		<!-- 订单详情 弹窗  end-->
				...
				<el-row :gutter="10">
					<el-col :span="12">
						...
						<el-form-item label="卡类型" prop="modelid">
							<el-select v-model="addForm.modelid" @visible-change="modelChange" :loading="typeListLoading" filterable placeholder="请选择卡类型" clearable>
								<el-option v-for="item in typeListData" :key="item.modelname" :label="item.modelname" :value="item.id">
								</el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="卡状态">
							<el-select disabled v-model="addForm.simcategory" placeholder="请选择卡状态">
							    <el-option key="ON" label="开机" value="ON"></el-option>
							    <el-option key="OFF" label="关机" value="OFF"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="在库状态">
							<el-select disabled v-model="addForm.status" @visible-change="stoStatusChange" :loading="stoStatusLoading" filterable placeholder="请选择在库状态" clearable>
		...
		<el-dialog title="卡类型设置" :modal-append-to-body="false" :visible.sync="typeFormVisible" width="60%" @close="typeCancel">
			...
			<el-table border :data="typeListData" ref="typeForm" max-height="800" v-loading="typeListLoading">
				...
				<el-table-column label="规格" align="center" width="120">
					<template slot-scope="scope">
						<div v-if="scope.$index == typeListData.length-1 && editable" >
							<el-form-item prop="modelspec" :rules="{required: true, message: '规格不能为空', trigger: 'blur'}">
								<el-select v-model="typeForm.modelspec" @visible-change="modelspecChange" :loading="modelspecLoading" filterable clearable>
									<el-option v-for="item in modelspecOptions" :key="item.dictdataname" :label="item.dictdatavalue" :value="item.dictdataname"></el-option>
								</el-select> 
							</el-form-item>
						</div>
						<el-select v-else-if="showEdit[scope.$index]" v-model="typeListData[scope.$index].modelspecname" @visible-change="modelspecChange" @change="chooseModelspec" :loading="modelspecLoading" filterable clearable>
							<el-option v-for="item in modelspecOptions" :key="item.dictdataname" :label="item.dictdatavalue" :value="item.dictdataname"></el-option>
						</el-select> 
				...
				<el-table-column label="运营商" align="center">
					<template slot-scope="scope">
						<div v-if="scope.$index == typeListData.length-1 && editable" >
							<el-form-item prop="operator" :rules="{required: true, message: '运营商不能为空', trigger: 'blur'}">
								<el-select v-model="typeForm.operator" @visible-change="operatorChange" :loading="operatorLoading" filterable clearable>
									<el-option v-for="item in operatorOptions" :key="item.dictdataname" :label="item.dictdatavalue" :value="item.dictdataname"></el-option>
								</el-select> 
							</el-form-item>
						</div>
						<el-select v-else-if="showEdit[scope.$index]" v-model="typeListData[scope.$index].operatorname" @visible-change="operatorChange" @change="chooseOperator" :loading="operatorLoading" filterable clearable>
							<el-option v-for="item in operatorOptions" :key="item.dictdataname" :label="item.dictdatavalue" :value="item.dictdataname"></el-option>
						</el-select> 
		...
		<el-dialog title="批量入库" :visible.sync="batchVisible" width="40%" @close="closePutIn">
		           	<el-form label-width="90px" :model="putInForm" ref="putInForm" :rules="putInRules">
		           		<el-form-item label="入库批次">
			                    <span class="batch">{{ putInForm.batchno }}</span>
			            </el-form-item>
		                        <el-form-item label="采购合同" prop="purchaseid">
		                            <el-select filterable placeholder="请选择采购合同" v-model="contractno" @focus="getStoPurchaseList" @change="chooseChange" value-key="contractno">
		                                <el-option v-for="(item, index) in purchaseList" :value="item" :label="item.contractno + '-' + item.contractname" :key="index"></el-option>
		            ...
		            <div slot="footer" class="dialog-footer">
		                ...
		                <el-button @click="batchSubmit" type="primary" :loading="addLoading">提交</el-button>
		            </div>
		</el-dialog>
