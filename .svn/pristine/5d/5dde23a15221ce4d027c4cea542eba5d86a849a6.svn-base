        ...
        confirmFinishInstall () {
            ...
            this.$refs.endForm.validate(async valid => {
                if (valid) {
                    ...
                    // 同一设备不允许重复添加
                    for(var i=0; i<this.endForm.prods.length; i++){
                        if (this.endForm.prods[i].prodnum == this.endForm.prods[i+1].prodnum){
                            this.$message.error('设备编号为 【'+this.endForm.prods[i].prodnum+'】 的设备重复！请重新选择！');
                            return;
                        }
                    }
                    let para = {
