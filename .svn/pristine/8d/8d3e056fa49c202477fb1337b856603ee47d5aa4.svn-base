...
<el-table :max-height="windowOutHeight-320" :data="todo" border ref="todeTable" highlight-current-row @expand-change="expandHandle" v-loading="listLoading" style="width: 100%;" @row-dblclick="editOrder">
            ...
            <el-tab-pane label="订单详情" name="1">
                <el-row>
                    ...
                    <el-col :span="6">
                        ...
                        <dl class="dllist">
                            <dt>初登日期:</dt>
                            <dd>{{ props.row.vehicleinfo.firstregisterdate ? Dayjs(props.row.vehicleinfo.firstregisterdate).format('YYYY-MM-DD') : '--' }}</dd>
                        </dl>
                        <dl class="dllist">
                            <dt>第一受益人:</dt>
...
<el-table-column v-if="isEmployee" label="操作" width="110" align="center" fixed="right">
    <template slot-scope="scope">
        <el-dropdown trigger="click" v-if="scope.row.iscancelled != '1'">
...
<el-dialog :title="installStep == '1' ? '开始安装信息' : '完成安装信息'" :visible.sync="finishInstallDialogVisible" :close-on-click-modal="false" @close="closeFinish">
    ...
    <el-form :model="startForm" ref="startForm" :rules="startFormRules" v-if="installStep == '1'">
        ...
        <el-col :span="6">
            <dl class="dllist">
                <dt>派单时间：</dt>
                <dd>{{ rowFinishData.vehicleinfo ? Dayjs(rowFinishData.vehicleinfo.assigndate).format('YYYY-MM-DD HH:mm') : '--' }}</dd>
            </dl>
            <dl class="dllist">
                <dt>预约时间：</dt>
    ...
    <el-form :model="endForm" ref="endForm" label-width="120px" :rules="endFormRules" v-else-if="installStep == '2'">
        ...
        <div v-for="(item,index) in prodCatagorys">
            <!-- 标题部分 -->
            <el-col :span="24" v-for="(item2,index2) in item">
                <!-- 配件 -->
                <div>
                    <p class="invoice-title" v-if="item2.cmsProduct.prodcategory == 'P_ACCESSORY'">{{ '配件' }}</p>
                    <el-card v-if="item2.cmsProduct.prodcategory == 'P_ACCESSORY'" shadow="never" class="space">
                        <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
                        <el-checkbox-group v-model="checkedParts" @change="handleCheckedPartsChange">
                            <el-checkbox v-for="item in parts" :label="item" :key="item.cmsProduct.id">{{item.cmsProduct.proddesc + '&emsp;X ' + item.prodqty}}</el-checkbox>
                        </el-checkbox-group>
                    </el-card>
                </div>
                <!-- 设备 -->
                <div>
                   <p class="invoice-title" v-if="item2.cmsProduct.prodcategory == 'P_WIRE'">{{ '有线设备' }}</p>
                    <p class="invoice-title" v-if="item2.cmsProduct.prodcategory == 'P_WIRELESS'">{{ '无线设备' }}</p>
                    
                    <span v-if="item2.cmsProduct.prodcategory != 'P_ACCESSORY'">
                        <!-- 选择设备型号 -->
                        <el-select v-model="prods[index2].prodmodelid" placeholder="请选择已安装的设备型号" class="row_select" @focus="modelChange(item2)" clearble :loading='modelloading'>
                            <el-option v-for="item in modellist" :key="item.id" :label="item.modelname" :value="item.id"> </el-option>
                        </el-select>
                        <!-- 添加设备按钮 -->
                        <el-popover ref="popover" v-if="prods[index2].prodmodelid != undefined" placement="bottom" width="720" trigger="click" @show="getProds(index2)">
                    ...
                    <!-- 设备信息 -->
                    <el-card shadow="never" v-if="show[index2] && item2.cmsProduct.prodcategory != 'P_ACCESSORY'" class="space">
                        <el-col :span="8">
                            <dl class="dllist">
                                <dt>设备型号：</dt>
                                <dd>{{ endForm.prods[index2].modelname }}</dd>
                            </dl>
                        </el-col>
                        <el-col :span="8">
                            <dl class="dllist">
                                <dt>设备编号：</dt>
                                <dd>{{ endForm.prods[index2].prodnum }}</dd>
                            </dl>
                        </el-col>
                        <el-col :span="8">
                            <dl class="dllist">
                                <dt>SIM卡号：</dt>
                                <dd>{{ endForm.prods[index2].simnum }}</dd>
                            </dl>
                        </el-col>
                        <el-col :span="8">
                            <dl class="dllist">
                                <dt>卡类型：</dt>
                                <dd>{{ endForm.prods[index2].simmodelname }}</dd>
                            </dl>
                        </el-col>
                        <el-col :span="11">
                            <el-form-item label="设备安装位置：" 
                            :prop="'prods.'+index2+'.installpositioncode'" :rules="endFormRules.installpositioncode">
                                <el-select filterable :loading="codeloading" @visible-change="changeInstallDecode" v-model="endForm.prods[index2].installpositioncode" placeholder="请选择设备安装位置">
                        ...
                        <el-col :span="5">
                            <el-popover placement="bottom" width="1000" trigger="click">
                                <div>信号检测结果:
                                    <el-button style="margin-left: 10px;" type="primary" size="mini" @click="getSignalInfo(endForm.prods[index2].prodnum)">再次检测
                                    </el-button>
                                </div>
                                ...
                                <el-button style="margin-left: 20px;" slot="reference" type="success" icon="el-icon-circle-check" round plain @click="getSignalInfo(endForm.prods[index2].prodnum)">信号检测 </el-button>
                            </el-popover>
                        </el-col>
                        <el-col :span="24">
                            <el-form-item label="设备安装图片："  :prop="'prods.'+index2+'.busiPictures'" :rules="endFormRules.busiPictures">
                                <el-upload
                                    name="file" class="avatar-uploader" 
                                    :headers="{Authorization: 'Bearer '+ token}" 
                                    :accept="accept" 
                                    :on-success="successInvoiceOne"
                                    @click.native="uploadPicIndex(index2)"
                                    action="/admin/atta/upload/picture"
                                    :show-file-list="false"
                                    v-model="endForm.prods[index2].busiPictures[0].piclink">
                                    <img v-if="endForm.prods[index2].busiPictures[0].piclink" :src="$store.state.IMG_URL+endForm.prods[index2].busiPictures[0].piclink" class="avatar">
                                    <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                                ...
                                <el-upload
                                    ...
                                    action="/admin/atta/upload/picture"
                                    :show-file-list="false"
                                    v-model="endForm.prods[index2].busiPictures[1].piclink">
                                    <img v-if="endForm.prods[index2].busiPictures[1].piclink" :src="$store.state.IMG_URL+endForm.prods[index2].busiPictures[1].piclink" class="avatar">
                                    <i v-else class="el-icon-plus avatar-uploader-icon"></i>
               ...
               <!--  <el-card shadow="never" v-if="show[index2] && item2.cmsProduct.prodcategory != 'P_ACCESSORY'" class="space">
                    ...
                    <el-col :span="11">
                        <el-form-item label="设备安装位置：" prop="installpositioncode">
                            <el-select filterable :loading="codeloading" @visible-change="changeInstallDecode" v-model="endForm.prods[index2].installpositioncode" placeholder="请选择设备安装位置">
                                <el-option v-for="item in InstallPositionArray" :key="item.dictdatavalue" :label="item.dictdatavalue" :value="item.dictdataname">
                                </el-option>
                            </el-select>
                    ...
                    <el-col :span="24">
                        <el-form-item label="设备安装图片："  prop="busiPictures">
                            <el-upload
                                name="file" class="avatar-uploader" 
                                :headers="{Authorization: 'Bearer '+ token}" 
                                :accept="accept" 
    ...
    <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="installStep == '1' ? nextStep(rowFinishData) : confirmFinishInstall()">{{ installStep == '1' ? '下一步' : '完成安装' }}</el-button>
    </span>
</el-dialog>
<!-- 完成安装 步骤 弹窗  end-->
