    ...
    <div class="order-process-box">
        ...
        <div style="flex-grow: 1;">
            <el-collapse v-model="activeNames" class="bdcoll rpShowimgDialog" ref="rpShowimgDialog">
                <el-collapse-item class="lefw10 wzpdt10" name="1" v-if="failedReasonList.length">
                    <template slot="title">未自动出单原因
                    </template>
                    <ul  style="margin-left: 10px; color: red">
                        <li v-for="(item, index) in failedReasonList" :key="index">
                            {{item}}
                        </li>
                    </ul>
                    <!--<span style="margin-left: 10px; color: red">未填写发动机号，未填写车辆购置价</span>-->
                </el-collapse-item>
