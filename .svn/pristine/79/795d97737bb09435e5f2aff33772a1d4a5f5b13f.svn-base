...
import {  cldeviceIsOnstate, getProdCatagorys, getOperationRecord, getNodesCount, getProdModels, getModelProds, setStartFormRules, getTodoInfoList, getInstallPositionCode, startInstall, finishInstall, validateVinVeh,deleteProcess,returnProccess  } from './service';
 ...
 export default {
        ...
        data() {
            return {
                ...
                endFormRules:{
                    ...
                    busiPictures :[{required:true,message:"请上传设备安装图片",trigger: 'blur'}],
                },
                prodData: [],//设备选择列表
                prodLoading:false,
                todo:[],//列表数据
        ...
        methods: {
            ...
            confirmFinishInstall(){
                ...
                this.$refs.endForm.validate(async valid => {
                    if (valid) {
                        const arr = this.endForm.parts.concat(this.endForm.prods);
                        if(arr.filter(item => item&&item.prodnum).length !== arr.length || arr.length !== this.prods.length) {
        ...
        // -----------------------------------------------列表----------------------------------------
            ...
            dateFormatter2(row,col){
                return util.formatDate.format(new Date(row.startDate), 'yyyy-MM-dd hh:mm');
            },
            //格式化后台传来的时间戳
            dateFormatter3(row,col){
                return util.formatDate.format(new Date(row.endDate), 'yyyy-MM-dd hh:mm');
            },
        // ----------------------------------------------下拉框------------------------------
        /**
         * 获取车辆颜色
