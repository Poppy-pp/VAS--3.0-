    ...
    data() {
        return {
            ...
            loseFormRules:{
                ...
                storagename: [{required: true,message: '请选择库房',trigger: 'blur'}],
            },
            ytotal: 0,
            ycurrentPage: 1,
            ypageSize: 15,
    ...
    methods: {
        ...
        loseEmpChange(r){
            ...
            getEmployeeInfoList().then((res) => {
                this.loseEmplist = res.data.data;
        ...
        chooseLoseEmp(val) {
            if (val == '') {//清空时
                this.loseForm.storageid = '';
                this.loseForm.storagename = '';
            }else{
                this.loseForm.lostuserid = val.userid;//遗失人userid-提交和查设备用
                this.loseForm.lostuid = val.id;//遗失人id-库房用
                this.loseForm.lostuser = val.employeename;//页面显示
            }
        },
        // 库房下拉
        stoChange(r){
            this.storages = [];
            if(!r || this.storages.length > 0) return;
            getStoList({limit:1000,id:this.loseForm.lostuid}).then((res) => {
                this.storages = res.data.data;
            });
        },
        // 选择仓库时
        chooseStorage (val) {
            this.loseForm.storageid = val.id;
            this.loseForm.storagename = val.storagename;
            this.chooseLoseProdData = [];
        },
        //点击 添加按钮
        addDeviceList(val) {
            if (this.loseForm.lostuserid == undefined || this.loseForm.lostuserid == '' ) {
                this.$message.warning('请先选择遗失人！');
                return;
            }
            if (this.loseForm.storageid == undefined || this.loseForm.storageid == '' ) {
        ...
        getDeciceListInfo() {
            let para = {
                ...
                userid:this.loseForm.lostuserid,
        ...
        comlist(checkData, event) {
            // 选择重复设备时
            let isRepeat = true;
            this.chooseLoseProdData.forEach((item,index) =>{
        ...
        // 获取配件下拉信息
        modelCascader(){
            getPurchaseCascader({storageid:this.loseForm.storageid}).then((res) => {
