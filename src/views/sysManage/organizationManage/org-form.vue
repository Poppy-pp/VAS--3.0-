<template>
    <el-form label-position="right" label-width="110px" :model="form" ref="form" v-loading="loading" :rules="rules">
        ...
        <el-form-item label="公司类型" prop="corpcategory">
            <el-select v-model="form.corpcategory" filterable :disabled="formEdit" placeholder="请选择公司类型"
                       clearable>
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                ...
                rules: {
                    ...
                    corpshortname: [
                        {required: true, message: '公司缩写必填', trigger: 'blur'},
                        {max: 4, message: '公司缩写不能大于4位', trigger: 'blur'},
        ...
        methods: {
            ...
            init () {
                let corplevel = this.data.corplevel + 1
            ...
            async corpcategoryChange (show) {
                if (!show) return
                try {
                    let dictvalue = 'CORPCATEGORY_INT'
                    if (this.form.corptype === 'EXTERNAL') {
                        dictvalue = 'CORPCATEGORY_EXT'
                    }
                    let param = {
                        page: 1,
                        limit: 10000,
            ...
            update () {
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        this.form.corpshortname = this.form.corpshortname.toLocaleUpperCase()
