                ...
                <template v-for="(item,index) in filters.domSearch">
                    <template v-if="index == 0">
                        <div style="display:inline-block;margin:0 10px 10px 0;">
                            <el-input class="noborder color icon nofocus" @keyup.native.ctrl.8="clearAll()" @keyup.native.13="handleQuery" placeholder="请输入查询内容"
                    ...
                    <template v-else>
                        <el-col :span="24">
                            <div style="display:inline-block;margin:0 10px 10px 0;">
                                <el-input class="noborder color icon nofocus" @keyup.native.ctrl.8="clearAll()" @keyup.native.13="handleQuery"
                ...
                <template slot-scope="scope">
                    <el-dropdown trigger="click" v-if="scope.row.iscancelled != '1'">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <el-dropdown-item v-if="scope.row.status == 2 || scope.row.status == 3" @click.native="passConstructionOrder(scope.$index, scope.row)">退 回
                            </el-dropdown-item>
                            <el-dropdown-item v-if="scope.row.status == 1 || scope.row.status == 2 || scope.row.status == 3 || scope.row.status == 4" @click.native="deleteOrder(scope.$index, scope.row)">废 单
                            </el-dropdown-item>
        ...
        <el-dialog :title="installStep == '1' ? '开始安装信息' : '完成安装信息'" :visible.sync="finishInstallDialogVisible" :close-on-click-modal="false"
            ...
            <el-form :model="startForm" ref="startForm" :rules="startFormRules" v-if="installStep == '1'">
                ...
                <el-row :gutter="20" v-if="!isSameVin">
                    ...
                    <el-col :span="16">
                        <p style="color:red;margin-top:40px;" v-if="isExistVin">系统已存在该车辆，若需变更信息，请勾选变更！&emsp;<el-checkbox size="mini" label="变更信息" border @change="changeOwnerVeh"></el-checkbox>
                        </p><!-- 系统存在 -->
                ...
                <el-row v-else style="height: 0;">
                    ...
                    <el-row :gutter="20">
                        ...
                        <el-col :span="8">
                            <el-form-item v-if="rowFinishData.vehicleinfo.hastheftinsurance == 1" label="初登日期" prop="firstregisterdate">
