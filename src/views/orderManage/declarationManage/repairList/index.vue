        ...
        type="border-card">
            ...
            <el-tab-pane label="操作记录" name="2">
                ...
                style="width: 100%">
                    ...
                    <el-table-column prop="endDate" :formatter="dateFormatter2" align="center" label="最后操作时间"> </el-table-column>
...
<el-dialog :title="installStep == '1' ? '开始维修信息' : '完成维修信息'" :visible.sync="finishInstallDialogVisible" :close-on-click-modal="false" @close="closeFinish">
    ...
    <el-form :model="startForm" ref="startForm" :rules="startFormRules" v-if="installStep == '1'">
        ...
        <el-col :span="6">
            ...
            <dl class="dllist">
                <dt>派单备注：</dt>
                <dd>{{ rowFinishData.clerkremarks }}</dd>
            </dl>
        </el-col>
        <el-col :span="6">
            <dl class="dllist">
    ...
    <el-form :model="endForm" ref="endForm" label-width="120px" :rules="endFormRules" v-else-if="installStep == '2'">
        <el-col :span="24">
            <span class="formTile">选择维修设备</span>
        </el-col>
        <div v-for="(item,index) in prodCatagorys" >
            <el-col :span="24">
                <p class="invoice-title">{{ item.stoPackinfoDto.promodelspecname + '设备' }}</p>
                <el-select v-model="item.actiontype" placeholder="请选择维修情况" class="row_select">
                    <el-option v-for="item in actionTypes" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                </el-select>
                <el-button v-if="ischange[index]"  type="warning" round class="updateBtn" icon="el-icon-close" @click="cancelChoose(index)">取消更换</el-button>
                <el-popover ref="popover" v-else-if="item.actiontype == 'CHANGE' || item.actiontype == 'LOST-INSTALL'" placement="bottom" width="720" trigger="click" @show="getProds(item,index)">
                ...
                <el-card shadow="never" class="space">
                    ...
                    <el-col :span="11">
                        <el-form-item v-if="ischange[index]" label="设备安装位置：" 
                             prop="installpositioncode" :rules="endFormRules.installpositioncode">
                            <el-select filterable :loading="codeloading" @visible-change="changeInstallDecode" v-model="item.installpositioncode" placeholder="请选择设备安装位置">
                                <el-option v-for="item in InstallPositionArray" :key="item.dictdatavalue" :label="item.dictdatavalue" :value="item.dictdataname">
                                </el-option>
                            </el-select>
                        </el-form-item>
                        <dl class="dllist" v-else>
                            <dt>设备安装位置：</dt>
                    ...
                    <el-col :span="5">
                        <el-popover placement="bottom" width="1000" trigger="click">
                            <div>信号检测结果:
                                <el-button style="margin-left: 20px;" slot="reference" :type="signalInfo.curonlinestatus == '离线' ? 'danger' : 'success'" :icon="signalInfo.curonlinestatus == '离线' ? 'el-icon-circle-close' : 'el-icon-circle-check'" round plain
                    ...
                    <el-col :span="24">
                        <el-form-item v-if="ischange[index]" label="设备安装图片：" prop="piclink" :rules="endFormRules.piclink">
        ...
        <el-input type="textarea" placeholder="请输维修备注信息" v-model="endForm.impremark"></el-input>
    </el-form>

    <span slot="footer" class="dialog-footer">
        <el-button @click="installStep == '1' ? cancelInstall() : previousStep()">{{ installStep == '1' ? '取 消' : '上一步' }}</el-button>
        <el-button type="primary" @click="installStep == '1' ? nextStep(rowFinishData) : confirmFinishInstall()">{{ installStep == '1' ? '下一步' : '完成维修' }}</el-button>
    </span>
</el-dialog>
<!-- 完成 步骤 弹窗  end-->
