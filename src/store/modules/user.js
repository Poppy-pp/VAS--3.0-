        ...
        getUserInfo ({commit}) {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    let name = '暂无'
                    let avatar = ''
                    if (response.data.employeeInfo) {//当有员工信息时
                        name = response.data.employeeInfo.employeename
                        avatar = response.data.employeeInfo.headiconpath
                    } else if(response.data.customerDTO) {
                        name = response.data.customerDTO.custname
                        avatar = response.data.customerDTO.headiconpath
                    }
